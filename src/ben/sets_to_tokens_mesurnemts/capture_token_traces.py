...
def main():
    ...
    for token_id in range(128):
        ...
        # Create test script
        create_test_script(token_id)
        
        # Run PIN trace
        if run_pin_trace(token_id):
            # Move the trace file
            if os.path.exists('detokenize_trace.out'):
                os.rename('detokenize_trace.out', f'token_traces/token_{token_id:03d}_addresses.txt')
                successful_traces += 1
                print(f"✓ Token {token_id} traced successfully")
            else:
                print(f"✗ Token {token_id}: trace file not created")
                failed_traces.append(token_id)
        else:
            failed_traces.append(token_id)
        
        # Clean up test script
        if os.path.exists(f'test_token_{token_id}.py'):
            os.remove(f'test_token_{token_id}.py')
